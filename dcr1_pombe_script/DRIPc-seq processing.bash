...
#clean data
java -jar ${trim}/trimmomatic-0.39.jar PE -threads $threads -phred33 $read_dir/Rawdata/${name}_R1_001.fastq.gz $read_dir/Rawdata/${name}_R2_001.fastq.gz -baseout $read_dir/trim_data/$name.trim.fq.gz LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:32 ILLUMINACLIP:/work/home/path/ref/adapter_sum.fa:2:30:10 > $read_dir/trim_data/$name.trim.log 2>&1 

#mapping

bowtie2 -x ${bowtie2_index} --phred33 -q --no-unal -1 $read_dir/trim_data/$name.trim_1P.fq.gz -2 $read_dir/trim_data/$name.trim_2P.fq.gz -p $threads -S $read_dir/bowtie2/$name.trim.sam > $read_dir/bowtie2/$name.trim_map.log 2>&1
...
# fwd_rev
#fwd
samtools view -b -f 128 -F 16 $read_dir/bowtie2/$name.trim.sort.rmdup.bam > $read_dir/bowtie2/fwd_rev/$name.fwd1.bam
samtools index $read_dir/bowtie2/fwd_rev/$name.fwd1.bam
samtools view -b -f 80 $read_dir/bowtie2/$name.trim.sort.rmdup.bam > $read_dir/bowtie2/fwd_rev/$name.fwd2.bam
...
#rev
samtools view -b -f 144 $read_dir/bowtie2/$name.trim.sort.rmdup.bam > $read_dir/bowtie2/fwd_rev/$name.rev1.bam
samtools index $read_dir/bowtie2/fwd_rev/$name.rev1.bam
samtools view -b -f 64 -F 16 $read_dir/bowtie2/$name.trim.sort.rmdup.bam > $read_dir/bowtie2/fwd_rev/$name.rev2.bam
...
factor=`echo "scale=5;1000000/$(samtools view -@ $threads -c $read_dir/bowtie2/$name.trim.sort.rmdup.bam)" | bc`
